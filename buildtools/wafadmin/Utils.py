...
import os, sys, string, errno, traceback, inspect, re, shutil, datetime, gc
from imp_compat import imp
...
def exec_command(s, **kw):
    ...
    if 'log' in kw:
        log_file = kw['log']
        del(kw['log'])
        # Python 3: Capture output and write as text to log
...
def python_24_guard():
    # Allow Python 3.x as well (patched for modern Python compatibility)
    if sys.hexversion < 0x20400f0:
        raise ImportError("Waf requires Python >= 2.4")

def ex_stack():
    exc_type, exc_value, tb = sys.exc_info()
    # Always include traceback for debugging Python 3 compatibility issues
...
def h_list(lst):
    m = md5()
    # Python 3: md5 update requires bytes
    m.update(str(lst).encode('utf-8'))
    return m.digest()
...
def readf(fname, m='r'):
    ...
    # Python 3 removed 'U' mode, convert 'rU' to 'r'
    if 'U' in m:
        m = m.replace('U', '')
    f = open(fname, m)
    try:
