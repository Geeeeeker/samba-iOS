...
# Python 3 compatibility: Define node_class at module level for pickling
# We use a factory to create properly named classes at module scope
_nodu_counter = [0]

def _make_nodu_class():
    """Create a new Nodu subclass that can be pickled (has a proper module-level name)"""
    _nodu_counter[0] += 1
    cls_name = 'Nodu_%d' % _nodu_counter[0]
    # Create the class in the module namespace
    cls = type(cls_name, (Node.Node,), {})
    # Register it in the module globals so pickle can find it
...
class BuildContext(Utils.Context):
    ...
    def __init__(self):
        ...
        # Python 3 compatibility: Create a named class at module level for pickling
        self.node_class = _make_nodu_class()
