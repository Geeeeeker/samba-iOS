...
def run_c_code(self, *k, **kw):
    ...
    # Python 3: Save original directory early, before any operations that might change it
    original_cwd = None
    try:
        original_cwd = os.getcwd()
    except OSError:
        # CWD doesn't exist - try to recover to srcdir
        original_cwd = self.srcdir if hasattr(self, 'srcdir') else '/'
    ...
    back = original_cwd if original_cwd else os.path.abspath('.')
