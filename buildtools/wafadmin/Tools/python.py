...
# Python 3.12+ removed distutils, use sysconfig instead
try:
    from distutils.sysconfig import get_config_var as _distutils_get_config_var
    from distutils.sysconfig import get_python_lib as _distutils_get_python_lib
    _USE_SYSCONFIG = False
except ImportError:
    import sysconfig
    _USE_SYSCONFIG = True

    def _distutils_get_config_var(name):
        """Compatibility wrapper for distutils.sysconfig.get_config_var"""
        return sysconfig.get_config_var(name)

    def _distutils_get_python_lib(plat_specific=False, standard_lib=False, prefix=None):
        """Compatibility wrapper for distutils.sysconfig.get_python_lib"""
        if standard_lib:
            if plat_specific:
                return sysconfig.get_path('platstdlib', vars={'base': prefix} if prefix else None)
            else:
                return sysconfig.get_path('stdlib', vars={'base': prefix} if prefix else None)
        else:
            if plat_specific:
                return sysconfig.get_path('platlib', vars={'base': prefix} if prefix else None)
            else:
                return sysconfig.get_path('purelib', vars={'base': prefix} if prefix else None)
...
def check_python_headers(conf, mandatory=True):
    ...
    try:
        # Get some python configuration variables using sysconfig (Python 3.12+) or distutils.sysconfig
        v = 'prefix SO SYSLIBS LDFLAGS SHLIBS LIBDIR LIBPL INCLUDEPY Py_ENABLE_SHARED MACOSX_DEPLOYMENT_TARGET'.split()
        # Try sysconfig first (Python 3.10+), fall back to distutils.sysconfig
        sysconfig_imports = [
            'try:',
...
def check_python_version(conf, minver=None):
    ...
    if result:
        ...
        if 'PYTHONDIR' in conf.environ:
            ...
            # Compatibility imports for Python 3.12+ (distutils removed)
            compat_imports = [
                'import sys',
                'try:',
                ...
                '        if standard_lib:',
                '            path_name = "platstdlib" if plat_specific else "stdlib"',
                '        else:',
                '            path_name = "platlib" if plat_specific else "purelib"',
                '        return sysconfig.get_path(path_name, vars={"base": prefix} if prefix else None)'
            ]
        ...
        if 'PYTHONARCHDIR' in conf.environ:
            ...
            # Reuse same compat_imports from above if available, otherwise define inline
            if 'compat_imports' not in dir():
                compat_imports = [
                    'import sys',
                    'try:',
                    '    from distutils.sysconfig import get_config_var, get_python_lib',
